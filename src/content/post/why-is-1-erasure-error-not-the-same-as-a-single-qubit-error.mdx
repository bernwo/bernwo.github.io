...
Whilst it is no surprise that eqns ($\ref{eqn:ket0L}$) and ($\ref{eqn:ket1L}$) are orthogonal to each other, i.e., $\langle0_\mathrm{L}|1_\mathrm{L}\rangle=0$, it is interesting to note that the state $|0_\mathrm{L}\rangle$ ($|1_\mathrm{L}\rangle$) is essentially the linear superposition of all 5-qubit states with even (odd) parity.
